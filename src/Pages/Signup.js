...
function Signup() {
  ...
  const sLabel = ["", "Weak", "Fair", "Good", "Strong", "Great"][strength];
  const sColor = ["", "#ef4444", "#f59e0b", "#f59e0b", "#3b82f6", "#1e40af"][strength];
            ...
            {success ? (
              <div className="success-container">
                ...
                <div className="success-message">
                  Account created successfully.<br />Taking you to login...
                </div>
              </div>
            ) : (
              <>
                <div className="form-heading">Let's get<br />started.</div>
                <div className="form-rule" />

                <div className="fields">
                  {/* Name Field */}
                  <div style={{ position: "relative", paddingBottom: errors.name ? "28px" : "0" }}>
                    <div className={`fl ${focused === "name" ? "active" : ""}${name ? " has-val" : ""}`}>
                      ...
                      />
                      {errors.name && <div className="fl-err-row">{errors.name}</div>}
                    </div>
                  </div>
                        ...
                        <div className="s-bars">
                          {[1, 2, 3, 4, 5].map(i => (
                            <div className="s-bar" key={i}>
                              <div
                                className={`s-fill ${strength >= i ? "on" : ""}`}
                                style={{ background: strength >= i ? sColor : "transparent" }}
                              />
                            </div>
                          ))}
                        </div>
                        <div className="s-lbl" style={{ color: sColor }}>{sLabel}</div>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <button className={`btn ${loading ? "loading" : ""}`} onClick={handleSignup}>
                  {loading ? (
                    <>Creating account <div className="spin" /></>
                  ) : (
                    <>Create Account →</>
                  )}
                </button>
                ...
                <div className="btm">
                  ...
                  <span className="btm-lnk" onClick={() => navigate("/login")}>Sign in →</span>
                </div>
              </>
            )}
          </div>
